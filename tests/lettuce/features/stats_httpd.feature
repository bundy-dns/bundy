Feature: bundy-stats-httpd module
    ...
    And wait for new bundy log message "GET / HTTP/1.0" 302 -
    And wait for new bundy log message "GET /bundy/statistics/xml/ HTTP/1.0" 200 -
    The last http response status code should be 200

    When I request the URL http://127.0.0.1:47811/no_such_url
    And wait for new bundy log message "GET /no_such_url HTTP/1.0" 404 -
