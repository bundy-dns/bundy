Feature: Xfrin incoming notify handling
    ...
    And wait for master log message STATS_STARTING

    And I have bundy running with configuration xfrin/retransfer_slave_notify.conf
    ...
    # be 0 or 1 here.  The reason is: if bundy-xfrout has started up and is
    # ready for a request from bundy-stats, then bundy-stats does request
    # to bundy-xfrout and the value results in 1. Otherwise if
    # bundy-xfrout is starting and isn't yet ready, then bundy-stats
    # doesn't request to bundy-xfrout and the value still remains to be the
    # default value(0).
    ...
    When I send bundy with cmdctl port 56174 the command Xfrout notify example.org IN
    Then wait for new master log message XFROUT_NOTIFY_COMMAND
    Then wait for new bundy log message AUTH_RECEIVED_NOTIFY
    Then wait for new bundy log message ZONEMGR_RECEIVE_NOTIFY
    # Note: The following waits should be for *new* log messages, or
    # they will match older log messages that were generated by AXFR
    # during startup.
    Then wait for new bundy log message XFRIN_XFR_TRANSFER_STARTED
    Then wait for new bundy log message XFRIN_TRANSFER_SUCCESS not XFRIN_XFR_PROCESS_FAILURE
    Then wait for new bundy log message ZONEMGR_RECEIVE_XFRIN_SUCCESS
    Then wait for new master log message NOTIFY_OUT_REPLY_RECEIVED
    ...
    When I query statistics zones of bundy module Xfrout with cmdctl port 56174
    ...
    # suggests this event.
    wait for new master log message XFROUT_RECEIVED_GETSTATS_COMMAND
    ...
    # wait until the last stats requesting is finished
    When I query statistics zones of bundy module Xfrin with cmdctl
    wait for new bundy log message XFRIN_RECEIVED_COMMAND
    last bundyctl output should not contain "error"

    When I query statistics of bundy module Xfrin with cmdctl
    ...
    Given I have bundy running with configuration xfrin/retransfer_master_v4.conf with cmdctl port 56174 as master
    And wait for master log message BUNDY_STARTED_CC
    And wait for master log message CMDCTL_STARTED
    And wait for master log message AUTH_SERVER_STARTED
    And wait for master log message XFROUT_STARTED
    ...
    And I have bundy running with configuration xfrin/retransfer_slave_notify_v4.conf
    And wait for bundy log message BUNDY_STARTED_CC
    And wait for bundy log message CMDCTL_STARTED
    And wait for bundy log message AUTH_SERVER_STARTED
    And wait for bundy log message XFRIN_STARTED
    ...
    """
    last bundyctl output should not contain Error
    Then wait for new master log message XFROUT_NEW_CONFIG_DONE
    ...
    Then wait for new bundy log message ZONEMGR_RECEIVE_XFRIN_FAILED not ZONEMGR_RECEIVE_XFRIN_SUCCESS
    Then wait for new master log message NOTIFY_OUT_REPLY_RECEIVED
    ...
    When I send bundy with cmdctl port 56174 the command Xfrout notify example.org IN
    Then wait for new master log message XFROUT_NOTIFY_COMMAND
    Then wait for new master log message NOTIFY_OUT_SENDING_NOTIFY
    Then wait for new master log message NOTIFY_OUT_TIMEOUT
    ...
    """
    last bundyctl output should not contain "error"
    And wait for new master log message XFROUT_STARTED

    A query for www.example.com to [::1]:56176 should have rcode REFUSED

    When I send bundy with cmdctl port 56174 the command Xfrout notify example.com IN
    Then wait for new master log message XFROUT_NOTIFY_COMMAND
    Then wait for new bundy log message AUTH_RECEIVED_NOTIFY_NOTAUTH
    Then wait for new master log message NOTIFY_OUT_REPLY_RECEIVED
    ...
    #
    When I send bundy the following commands with cmdctl
    """
    ...
    """
    last bundyctl output should not contain "error"
    And wait for new bundy log message ZONEMGR_STARTED
    ...
    Then wait for new bundy log message AUTH_RECEIVED_NOTIFY
    Then wait for new bundy log message ZONEMGR_RECEIVE_NOTIFY
    Then wait for new bundy log message ZONEMGR_ZONE_NOTIFY_NOT_SECONDARY
    Then wait for new master log message NOTIFY_OUT_REPLY_RECEIVED
    ...
    When I send bundy the following commands with cmdctl
    """
    config remove Init/components bundy-zonemgr
    config commit
    """
    last bundyctl output should not contain "error"
    ...
    When I send bundy with cmdctl port 56174 the command Xfrout notify example.org IN
    Then wait for master log message XFROUT_NOTIFY_COMMAND
    Then wait for new bundy log message AUTH_RECEIVED_NOTIFY
    Then wait for new bundy log message AUTH_ZONEMGR_NOTEXIST not AUTH_ZONEMGR_ERROR
    Then wait for master log message NOTIFY_OUT_TIMEOUT not NOTIFY_OUT_REPLY_RECEIVED
    ...
    #
    When I send bundy the command Xfrin retransfer example.org IN
    Then wait for new bundy log message XFRIN_CONNECT_MASTER
    Then wait for new bundy log message ZONEMGR_RECEIVE_XFRIN_FAILED
    ...
    # wait until the last stats requesting is finished
    wait for new bundy log message STATS_SEND_STATISTICS_REQUEST
    wait for new bundy log message XFRIN_RECEIVED_COMMAND

    When I query statistics socket of bundy module Xfrin with cmdctl
