Feature: Xfrin incoming notify handling
    ...
    Then wait for new bundy stderr message AUTH_RECEIVED_NOTIFY
    Then wait for new bundy stderr message ZONEMGR_RECEIVE_NOTIFY
    # Note: The following waits should be for *new* log messages, or
    # they will match older log messages that were generated by AXFR
    # during startup.
    Then wait for new bundy stderr message XFRIN_XFR_TRANSFER_STARTED
    Then wait for new bundy stderr message XFRIN_TRANSFER_SUCCESS not XFRIN_XFR_PROCESS_FAILURE
    Then wait for new bundy stderr message ZONEMGR_RECEIVE_XFRIN_SUCCESS
    ...
    wait for new bundy stderr message XFRIN_RECEIVED_COMMAND
    last bundyctl output should not contain "error"
    ...
    Then wait for new bundy stderr message ZONEMGR_RECEIVE_XFRIN_FAILED not ZONEMGR_RECEIVE_XFRIN_SUCCESS
    Then wait for new master stderr message NOTIFY_OUT_REPLY_RECEIVED
    ...
    Then wait for new master stderr message XFROUT_NOTIFY_COMMAND
    Then wait for new bundy stderr message AUTH_RECEIVED_NOTIFY_NOTAUTH
    Then wait for new master stderr message NOTIFY_OUT_REPLY_RECEIVED
    ...
    """
    last bundyctl output should not contain "error"
    And wait for new bundy stderr message ZONEMGR_STARTED
    ...
    Then wait for new bundy stderr message AUTH_RECEIVED_NOTIFY
    Then wait for new bundy stderr message ZONEMGR_RECEIVE_NOTIFY
    Then wait for new bundy stderr message ZONEMGR_ZONE_NOTIFY_NOT_SECONDARY
    Then wait for new master stderr message NOTIFY_OUT_REPLY_RECEIVED
    ...
    Then wait for new bundy stderr message AUTH_RECEIVED_NOTIFY
    Then wait for new bundy stderr message AUTH_ZONEMGR_NOTEXIST not AUTH_ZONEMGR_ERROR
    ...
    Then wait for new bundy stderr message XFRIN_CONNECT_MASTER
    Then wait for new bundy stderr message ZONEMGR_RECEIVE_XFRIN_FAILED
    ...
    # wait until the last stats requesting is finished
    wait for new bundy stderr message STATS_SEND_STATISTICS_REQUEST
    wait for new bundy stderr message XFRIN_RECEIVED_COMMAND
